...
// This is the action to download a previously generated PDF. It remains unchanged.
...
// --- THIS IS THE ACTION CALLED BY YOUR FORM ---
export async function handleSingleContractSubmission(
  data: Trec14ContractData
): Promise<{ success: boolean; error?: string; pdfBytes?: number[]; fileName?: string }> {
  try {
    const user = await getUser();
    if (!user) {
    ...
    if (!teamData) {
        return { success: false, error: 'Could not find team information.' };
    }
    ...
    if (team.contractCredits < 1) {
      return { success: false, error: 'Insufficient credits. Please purchase more to generate a contract.' };
    }
    ...
    const newContractId = await db.transaction(async (tx) => {
      ...
      await tx
        .update(teams)
        .set({ contractCredits: team.contractCredits - 1 })
        .where(eq(teams.id, team.id));
      
      return newContract.id;
    });

    // Generate the PDF using the existing action logic, now that the contract is saved
    ...
    return {
      success: true,
      pdfBytes: Array.from(pdfBytes), // Convert Uint8Array to a plain array for server->client transfer
      fileName: `TREC_Contract_${data.property?.address?.replace(/[\s,]/g, '_') || newContractId}.pdf`,
    };
